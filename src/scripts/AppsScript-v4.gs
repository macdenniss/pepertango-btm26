...
// Brutia Sheet:  https://docs.google.com/spreadsheets/d/12a0vS7DPpqRrEKf2P8Obk5Uw1tvYATiLdPW3GJrX6Q4
...
var BRUTIA = {
  SHEET_ID:         '12a0vS7DPpqRrEKf2P8Obk5Uw1tvYATiLdPW3GJrX6Q4',
  SHEET_ISCRITTI:   'Iscritti',
  SHEET_LOG:        'Log',
  EMAIL_DANILO:     'pepertango@gmail.com',
  ...
  PACCHETTI: {
    'Calabria Gold':   { notti:3, prezzo:450, anticipo:225, saldo:225, link_anticipo:'', link_saldo:'' },
    'Calabria Silver': { notti:0, prezzo:300, anticipo:150, saldo:150, link_anticipo:'', link_saldo:'' },
    'Calabria Bronze': { notti:0, prezzo:150, anticipo:75, saldo:75, link_anticipo:'', link_saldo:'' }
  },

  LEZIONI: {
    'galdi_lorena_venerdi': 35,
    'galdi_lorena_sabato': 35,
    'galdi_lorena_domenica': 35
  },
  ...
  PRIORITA_PACCHETTI: ['Calabria Gold', 'Calabria Silver', 'Calabria Bronze'],
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'brutia_registrazione':   return brutiaRegistrazione(data);
...
// BRUTIA - Gestione registrazioni dal modulo online
function brutiaRegistrazione(data) {
  try {
    // Validazione dati
    if (!data.nome || !data.cognome || !data.email || !data.telefono) {
      return jsonResponse({ ok: false, error: 'Campi obbligatori mancanti' });
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) {
      return jsonResponse({ ok: false, error: 'Email non valida' });
    }
    ...
    if (data.packages && Array.isArray(data.packages)) {
      data.packages.forEach(function(pkg) {
        if (BRUTIA.PACCHETTI[pkg]) {
          totale += BRUTIA.PACCHETTI[pkg].prezzo;
          articoli.push(pkg + ': €' + BRUTIA.PACCHETTI[pkg].prezzo);
        }
      });
    }
    ...
    if (data.lessons && Array.isArray(data.lessons)) {
      data.lessons.forEach(function(lesson) {
        if (BRUTIA.LEZIONI[lesson]) {
          totale += BRUTIA.LEZIONI[lesson];
          articoli.push(lesson + ': €' + BRUTIA.LEZIONI[lesson]);
        }
      });
    }

    // Milonghe
    if (data.milongas && Array.isArray(data.milongas)) {
      data.milongas.forEach(function(milonga) {
        if (BRUTIA.MILONGHE[milonga]) {
          totale += BRUTIA.MILONGHE[milonga];
          articoli.push(milonga + ': €' + BRUTIA.MILONGHE[milonga]);
        }
      });
    }

    if (totale === 0) {
      return jsonResponse({ ok: false, error: 'Nessun articolo selezionato' });
    }

    // Salva nel foglio
    var ss = getSpreadsheet('Brutia');
    var sheet = getOrCreateSheet(ss, BRUTIA.SHEET_ISCRITTI);

    var newRow = [
      'BRU-' + new Date().getTime(),  // ID
      new Date(),                      // Data iscrizione
      data.nome,                       // Nome
      data.email,                      // Email
      data.telefono,                   // Telefono
      '',                              // Città
      '',                              // Paese
      data.role || 'Non specificato',  // Ruolo
      data.packages ? data.packages[0] : '',  // Pacchetto principale
      data.partner_nome || '',         // Partner
      '',                              // Legacy
      'Registrazione online',          // Note
      'In attesa',                     // Stato
      'Pagamento in sospeso',          // Pagamento
      '',                              // Camera
      '',                              // Tipo camera
      'No',                            // Email conf
      'No',                            // Email anticipo
    ...
    var emailBody = 'Ciao ' + data.nome + '!\n\n' +
      'Grazie per la registrazione a Brutia Tango Fest 2026!\n\n' +
      'Articoli selezionati:\n' + articoli.join('\n') + '\n\n' +
      'Totale: €' + totale + '\n\n' +
      'Ti contatteremo a breve con i dettagli per il pagamento.\n\n' +
      'Brutia Tango Fest Team';

    MailApp.sendEmail(data.email, 'Registrazione Brutia Tango Fest 2026', emailBody);

    return jsonResponse({
      ok: true,
      message: 'Registrazione ricevuta! Totale: €' + totale,
      id: newRow[0]
    });

  } catch (err) {
    logErrore('brutiaRegistrazione', err.toString());
    return jsonResponse({ ok: false, error: err.toString() });
  }
}

// ✅ FIX: Validazione lato server
function validateFormData(data) {
  ...
  if (!data.nome || String(data.nome).trim().length < 2) {
    throw new Error('Nome obbligatorio (minimo 2 caratteri)');
  }

  // Telefono opzionale ma se fornito deve essere valido
  if (data.telefono && !/^[0-9+\-\s()]{5,}$/.test(data.telefono)) {
    throw new Error('Telefono non valido');
  }

  // Pacchetto deve essere fornito
  if (!data.pacchetto || String(data.pacchetto).trim().length === 0) {
    throw new Error('Pacchetto obbligatorio');
  }
...
function gestisciNuovaIscrizione(data, evento) {
  // ✅ FIX: Valida i dati prima di processare
...
function getOrCreateSheet(ss, name) {
  ...
  if (!sheet) {
    ...
    // Se è il foglio Iscritti, aggiungi le intestazioni
    if (name === 'Iscritti') {
      var headers = [
        'ID', 'Data Iscrizione', 'Nome', 'Email', 'Telefono', 'Città', 'Paese', 'Ruolo',
        'Pacchetto', 'Compagno Ballo', 'Compagno Legacy', 'Note', 'Stato', 'Pagamento',
        'Camera', 'Tipo Camera', 'Email Conf', 'Email Anticipo', 'Email Saldo', 'Num Progr',
        'Priorità', 'Tipologia Stanza', 'Compagni Stanza', 'Cognome', 'Data Nascita', 'Facebook'
      ];
      sheet.appendRow(headers);
      sheet.getRange(1, 1, 1, headers.length).setBackground('#7b2ff7').setFontColor('#fff').setFontWeight('bold');
      sheet.setFrozenRows(1);
    }
  }
  return sheet;
